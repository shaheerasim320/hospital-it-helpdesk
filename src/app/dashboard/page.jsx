...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const mockUser = {
      ...
      role: "it", // Change this to "admin" or "it" to test different roles
    ...
    if (mockUser.role === "it") {
      fetchAssignedTickets(mockUser.email)
      fetchTicketStats()
    } else {
      fetchRecentTickets(mockUser.email)
    }
  }, [router])
  ...
  const getQuickActions = (role) => {
    if (role === "it") {
      ...
      // Default for doctors, nurses, staff
      return [
        {
                ...
                <div className="flex items-center space-x-2">
                  <span
                    className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
                      userRole === "it" ? "bg-blue-100 text-blue-800" : "bg-green-100 text-green-800"
                    }`}
                  >
